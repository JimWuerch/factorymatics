...
// helper function for PlayerData usage
  ...
  static List<SpendHistory> getPayments(int needed, ResourceType neededResource, ResourcePool sourcePool, List<ConverterBaseProduct> products) {
    var paths = <SpendHistory>[];

    // create our mapping of converters to identifiers so we can de-dup later
    var prodIds = <ConverterBaseProduct, int>{};
    var idToProd = <int, ConverterBaseProduct>{};
    var prodCount = 0;
    for (var prod in products) {
      idToProd[1 << prodCount] = prod;
      prodIds[prod] = 1 << prodCount;
      prodCount++;
    }
        ...
        for (var rt in ResourceType.values) {
          ...
          if (rt == neededResource || neededResource == ResourceType.any) {
            for (var count = 0; count < sourcePool.count(rt); ++count) {
              ...
              idToProd[1 << prodCount] = spenders.last;
              prodIds[spenders.last] = 1 << prodCount;
              prodCount++;
            }
          }
        }
      }
      _getSpendPaths(paths, products, spenders, sourcePool, SpendHistory(), needed, neededResource, prodIds);
    } else {
      _getConvertersPaths(paths, products, sourcePool, ResourcePool(), SpendHistory(), needed, neededResource, prodIds);
    }
  ...
  static void _getSpendPaths(List<SpendHistory> paths, List<ConverterBaseProduct> conv, List<SpendResourceProduct> spenders, ResourcePool inputPool, SpendHistory history,
      int needed, ResourceType neededResource, Map<ConverterBaseProduct, int> prodIds) {
    if (needed == 0) {
      return;
    }
...
// recursively try all permutations of products
  static void _getConvertersPaths(List<SpendHistory> paths, List<ConverterBaseProduct> conv, ResourcePool inputPool, ResourcePool outputPool, SpendHistory history, int needed,
      ResourceType neededResource, Map<ConverterBaseProduct, int> prodIds) {
    for (var c in conv) {
        ...
        if (inputPool.count(cv.source) > 0 || outputPool.count(cv.source) > 0) {
          ...
          for (var rt in ResourceType.values) {
            ...
            if (rt != neededResource || neededResource == ResourceType.any) {
              ...
              var found = false;
              for (var p in conv) {
                if (p.productType == ProductType.doubleResource && p.sourceResource == rt) {
                  found = true;
                  break;
                }
              }
            ...
            if (inputPool.count(cv.source) > 0) {
              ip2.sub1(cv.source);
              history2.add(UsedProduct(newProd, true, prodIds[c]));
            } else {
              op2.sub1(cv.source);
              history2.add(UsedProduct(newProd, false, prodIds[c]));
            }
            op2.add1(rt);
...
// recursively try all permutations of products
  static void _findMaxResources(ResourcePool max, List<ConverterBaseProduct> conv, ResourcePool inputPool, ResourcePool outputPool) {
    for (var c in conv) {
      if (c.productType == ProductType.convert) {
        var cv = c as ConvertProduct;
        if (inputPool.count(cv.source) > 0 || outputPool.count(cv.source) > 0) {
